...
${u.blue("Please consider disable `cacheHTML` and `cachePic`, or set `maxSize` and `maxPicSize` option.\n")}`)})},m=(e,a)=>x(a)?a.replace(/^\//,e):a,M=(e,a)=>(a.icons&&(a.icons=a.icons.map(n=>({...n,src:m(e,n.src)}))),a.shortcuts&&(a.shortcuts=a.shortcuts.map(n=>({...n,icons:n.icons?.map(t=>({...t,src:m(e,t.src)}))||[],url:m(e,n.url)}))),a.screenshots&&(a.screenshots=a.screenshots.map(n=>({...n,src:m(e,n.src)}))),a),F=(e,a)=>{a.favicon&&(a.favicon=m(e,a.favicon)),a.apple&&(a.apple.icon&&(a.apple.icon=m(e,a.apple.icon)),a.apple.maskIcon&&(a.apple.maskIcon=m(e,a.apple.maskIcon))),a.msTile&&a.msTile.image&&(a.msTile.image=m(e,a.msTile.image)),a.manifest&&(a.manifest=M(e,a.manifest))},K=(e,a="/",n=[])=>{const t=[],r=[];n.forEach(o=>{o[0]==="meta"?t.push(o[1].name):o[0]==="link"&&r.push(o[1].rel)});let l="";const i=(o,p,b={})=>{r.includes(o)||n.push(["link",{rel:o,href:p,...b}])},s=(o,p,b={})=>{t.includes(o)||n.push(["meta",{name:o,content:p,...b}])};if(e.favicon&&i("icon",e.favicon),e.manifest?.icons){const{icons:o}=e.manifest;o.length>0&&(l=o[0].src,e.manifest.icons.map(p=>{p.type?i("icon",p.src,{type:p.type,sizes:p.sizes}):i("icon",p.src,{sizes:p.sizes})}))}return i("manifest",`${a}manifest.webmanifest`,{crossorigin:"use-credentials"}),s("theme-color",e.themeColor||"#46bd87"),v(e.apple)&&(e.apple.icon||l)?(i("apple-touch-icon",e.apple.icon||l),s("apple-mobile-web-app-capable","yes"),s("apple-mobile-web-app-status-bar-style",e.apple.statusBarColor||"black"),e.apple.maskIcon&&i("mask-icon",e.apple.maskIcon,{color:e.themeColor||"#46bd87"})):e.apple!==!1&&l&&(i("apple-touch-icon",l),s("apple-mobile-web-app-capable","yes"),s("apple-mobile-web-app-status-bar-style","black")),v(e.msTile)&&(e.msTile.image||l)?(s("msapplication-TileImage",e.msTile.image||l),s("msapplication-TileColor",e.msTile.color||e.themeColor||"#46bd87")):e.msTile!==!1&&l&&(s("msapplication-TileImage",l),s("msapplication-TileColor",e.themeColor||"#46bd87")),s("viewport","width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover"),n},_=(e,a)=>{let n="",t="";return a.update==="hint"?(n+=`import SWHintPopup from "${a.hintComponent||`${k}components/SWHintPopup.js`}";
`,t+=`SWHintPopup,
...
`)},U=(e={},a=!0)=>n=>{a&&B(e),C(n,g,"2.0.0-rc.0"),n.env.isDebug&&c.info("Options:",e);const{base:t,shouldPrefetch:r=!0}=n.options;e.appendBase&&F(t,e),r===!0&&c.warn('The plugin will register service worker to handle assets, so we recommend you to set "shouldPrefetch: false" in VuePress config file.');const l=L(n,e);return n.options.head=K(e,t,n.options.head),w(n,{id:"hope"}),{name:g,define:()=>({PWA_LOCALES:P({app:n,name:g,default:f,config:e.locales}),SW_FORCE_UPDATE:e.update==="force",SW_PATH:e.swPath||"service-worker.js"}),extendsBundlerOptions:(i,s)=>{T(i,s,["mitt","register-service-worker"]),A(i,s,"vuepress-shared"),O(i,s,{path:"/manifest.webmanifest",response:async(o,p)=>(p.setHeader("Content-Type","application/manifest+json"),JSON.stringify(await l)),errMsg:"Unexpected manifest generate error"})},onGenerated:async i=>{await $(i,l),await H(i,e)},clientConfigFile:i=>_(i,e)}};export{f as pwaLocales,U as pwaPlugin};
